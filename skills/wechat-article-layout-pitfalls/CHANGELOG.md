...
- 【事故】2026-09-02 误用 `draft/add` 新建 + 删除旧稿的"重传"方式改稿，把运营者在后台手动调整过的头图/标题/摘要/文字全部删掉，且删除不可恢复。阿成要求：**任何已推送草稿再修改必须增量更新，不能全覆盖。**
- 【变更】`scripts/upload_to_draft.py`：
  - 台账加锁——`MANIFEST`（`.wx_draft_manifest.json`）记住源文件名 → media_id + 内容基线 hash；已推送过的源文件再次裸 `add` 会**拒绝**（exit 2）并红字提示改走增量更新。
  - 自动备份——`draft/update` 前先把草稿箱当前内容备份到 `draft-backups/`（json + html，人工版永不丢）。
  - 人工编辑检测——更新前 `draft/get` 比对内容 sha1 与基线，不一致＝后台被人工改过，默认**拒绝推送**；把人工修改 diff 出来合并进本地源后再 `--force` 放行。
  - 字段沿用——增量更新时不传 `--title/--author/--digest/--cover/--source-url` 就沿用草稿箱当前值，后台改过的标题/摘要/封面天然保留，只替换正文。
  - 新增 `--export-current`（只备份+登记基线、不推送）与 `--update`（`--update-media-id` 别名）。
  - 响应解析统一 `_jresp()` 显式 UTF-8 解码（修复 requests 误判 latin-1 导致 45110 author 超限）。
- 【变更】`SKILL.md`：version 0.6.1 → 0.7.0，B 节补"改稿红线自动化"说明；文档 `references/draft-api-upload.md` 同步更新。
- 【影响面】强化防误操作，不改变正常链路；首次推送行为不变，更新已有草稿多了一层层保护。阿成 2026-09-02 拍板。

## 0.6.1 (2026-08-20)

**移除脚本自设的 20,000 字符正文校验，正文限制回归微信官方口径：体积 <1MB，无字符数上限。**

- 【实证】2026-08-20「刑事辩护全流程·Pro版」排版稿 38,102 字符（UTF-8 46.6KB），为旧字符上限的 1.9 倍，直调 `draft/add` 微信照收、回读正文完整——20,000 字符线不是微信政策，是 `upload_to_draft.py` 初版自设的保守值。
- 【变更】
  - `scripts/upload_to_draft.py`：删除 `MAX_CONTENT_CHARS` 常量与字符数校验；保留 `MAX_CONTENT_BYTES`（1MB）并把报错文案改为注明"微信官方上限"；常量区留注释记录实证来源。
  - `references/draft-api-upload.md`：「content 字段的约束」改为"总大小 <1MB、没有单独的字符数上限"；「校验顺序」坑从"撑爆 20000 字符校验"改写为"撑爆 1MB 体积校验"。
  - `SKILL.md`：version 0.6.0 → 0.6.1，致谢补本版来源。
- 【影响面】只放宽校验，不改变上传链路、换图逻辑、回读核验；38K 字符长文从此可走脚本标准入口，无需再绕校验。阿成 2026-08-20 拍板。
