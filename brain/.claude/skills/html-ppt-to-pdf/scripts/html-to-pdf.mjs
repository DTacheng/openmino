...
await page.addStyleTag({
  content: `
    ...
    ${slideSelector} {
      ...
      /* Do NOT force display:block here — many decks set the slide itself to
         display:flex with justify-content/align-items to center the slides
         own content. Forcing block kills that centering and also kills any
         flex:1 children (e.g. shot-frame style flex:1) which then expand to
         natural size and blow up the page. We only override display when the
         original is none, done in the JS pass below. */
...
await page.evaluate((sel) => {
  document.querySelectorAll(sel).forEach((s) => {
    ...
    // If the source CSS used `display:none` to hide non-active slides, our
    // visibility/opacity overrides aren't enough — force a sane display value.
    // Most decks use flex on the slide itself; fall back to block only if no
    // inline/computed style hints flex.
    const cd = getComputedStyle(s).display;
    if (cd === 'none') s.style.setProperty('display', 'flex', 'important');
