...
├── brain/                              # Second Brain (shared knowledge base, junction)
│   ├── _index.md                       # Knowledge index
│   ├── sources/                        # External facts / archives
...
brain/ 是阿成的跨工作空间共享知识库，通过 junction 软链挂在这里，物理位置在 mino 工区。所有工作空间（mino、法律元力、培训等）都读写同一份。

- `brain/_index.md` — 知识库总索引（所有条目的目录和格式规范）
- `brain/sources/` — **客观资料库**（外部事实，有链接可验证，重要资料存档到 archive/）
- `brain/concepts/` — 概念卡片（我们的理解和提炼）
- `brain/insights/` — 洞见记录（我们的判断和观察）
- `brain/resources/` — 内部资源索引（指向各工作空间具体文件的路径指针）
- `brain/decisions/` — 关键决策记录及其理由

### 客观 vs 主观

- `sources/` 存外部事实（数据、报告、文章），有出处可验证
- `concepts/` `insights/` `decisions/` 存我们的总结和判断，通过 `refs:` 引用 sources/
- 不混淆"数据说的"和"我们觉得的"

### 知识条目格式

每个条目用 YAML frontmatter。详见 `brain/_index.md`。
正文末尾用 `[[文件名]]` 标记相关条目实现双向链接。

### 关联工作空间

| 工作空间 | 路径 | 用途 | 住户 |
|---------|------|------|------|
| mino | `C:\Users\PC\.myagents\projects\mino` | 阿成的私人工作区 | 小皮 🦊 |
| 法律元力 | `D:\BaiduSyncdisk\产品经理\元力工厂-For.Agents\法律元力` | 产品项目 — 法律 Agent 站点 | — |
| 元典开放平台 | `D:\BaiduSyncdisk\产品经理\元力工厂-For.Agents\元典开放平台` | 产品项目 — 面向开发者的法律 API 开放平台 | — |
| 培训 | `D:\BaiduSyncdisk\方案\培训` | 演讲与培训准备 | — |
| 效率创意 mino | `D:\效率创意\mino` | 效率工具、创意试验场 | 小飞（你） |
